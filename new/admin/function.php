...
<?php
	...
	if(isset($_POST['name']) && isset($_POST['desc']) && isset($_POST['desc']) && isset($_POST['content'])) {
		...
		if(validString('function name',$_POST['name'])
			&& validString('function description',$_POST['desc'])
	...
	try {
		...
		if($count) { // there are functions in the db
			...
			foreach($dbh->query($sql) as $row) {
				...
				<td>
				<textarea name="desc" onblur="update(\''.$row['idfunctions'].'\',this)">'.$row['desc'].'</textarea>
				</td>
				<td>
				<textarea name="content" onblur="update(\''.$row['idfunctions'].'\',this)">'.$row['content'].'</textarea>
				</td>';
				if($sudo) echo '<td class="center"><a href="function.php?del='.$row['idfunctions'].'">Delete &#187;</td>';
				echo '</tr>';
			}
			echo '</table>';
		}
