<?php if(!isset($common_included_flag)) { require_once '/home/a6325779/public_html/old/includes/common.php'; } ?>
<meta name="keywords" content="Hills Road,HRSFC,myhrsfc,Hills Road Sixth Form College,Cambridge,Sixth Form,Student Council" />
<meta name="robots" content="index, follow" />
<meta http-equiv="content-type" content="text/html; charset=UTF-8" />
<meta name="geo.position" content="52.188179;0.136028" />
<meta name="geo.placename" content="Hills Road Sixth Form College, Cambridge, United Kingdom" />
<meta name="geo.region" content="GB-CAM" />
<link rel = "icon" type = "image/x-icon" href = "http://www.myhrsfc.co.uk/favicon.ico" />
        
<!-- 

	 Copyright 2012, Andrew J. Watts
	 All Rights Reserved
     Extended permission for use by HRSFC Student Council between April 2013 - March 2014
    
     Dw, I chatted with him and he said it was alright for us to use it (2014-2015), though I'm pretty sure this disclaimer isn't that legit and it's been changed quite a bit since...
     
     Many thanks to Luis Zuno ("Ansimuz") for the javascripting and the fiddly coding!
     ^ Lovely work mate!
     
     LEGAL DISCLAIMER:
     
     Between April 2014 and March 2015, Hills Road Sixth Form College and the Hills Road Student community are authorised to use this website, it's content and it's design(s).
     
     From March 2015 this website will be FULL PROPERTY of the author, Andrew J Watts. Hills Road Sixth Form College will have no right or obligation to have access to or to use this website, it's content and it's design(s) from this date onwards unless agreed with any future Student Council.
  
     END OF DISCLAIMER
     
     -->

		
		<!--[if lt IE 9]>
			<script src="http://html5shim.googlecode.com/svn/trunk/html5.js"></script>
		<![endif]-->
		<link rel="stylesheet" media="all" type="text/css" href="/css/style1.css"/>
		<meta name="viewport" content="width=device-width, initial-scale=1"/>
		<!-- Adding "maximum-scale=1" fixes the Mobile Safari auto-zoom bug: http://filamentgroup.com/examples/iosScaleBug/ -->
		
		<!-- JS -->

		<script src="/js/jquery-1.6.4.min.js" type="text/javascript"></script>
		<script src="/js/css3-mediaqueries.js" type="text/javascript"></script>
		<!--  <script src="js/less-grid-4.js" type="text/javascript"></script> -->
		<script src="/js/custom.js" type="text/javascript"></script>
		<script src="/js/tabs.js" type="text/javascript"></script>
		
		<!-- Masonry: dynamic style helper 
		<script src="js/masonry.min.js" type="text/javascript"></script>
		<script src="js/imagesloaded.js" type="text/javascript"></script>
		 ENDS Masonry -->


		<!-- superfish: drop down menu -->
		<link rel="stylesheet" media="screen" type="text/css" href="/css/superfish.css" /> 
		<script src="/js/superfish-1.4.8/js/superfish.js" type="text/javascript"></script>
		<script src="/js/superfish-1.4.8/js/hoverIntent.js" type="text/javascript"></script>
		<script src="/js/superfish-1.4.8/js/supersubs.js" type="text/javascript"></script>
		<!-- ENDS superfish -->

<?php
# recommendation: replace all 4 superfish imports with custom menu css (just css transitions):
#		<link rel="stylesheet" type="text/css" media="screen" href="css/nav.css" /> 
?>
		
		<!--[if IE 6]>
		<link rel="stylesheet" href="css/ie6-hacks.css" type="text/css" media="screen" />
		<script type="text/javascript" src="js/DD_belatedPNG.js"></script>
			<script>
          		/* EXAMPLE */
          		DD_belatedPNG.fix('*');
        	</script>
		<![endif]-->
		
		<!-- Lessgrid -->
		<link rel="stylesheet" type="text/css" media="all" href="/css/lessgrid.css"/>
		
		<!-- modernizr -->
		<script src="/js/modernizr.js" type="text/javascript"></script>
    	<script>
			(function(i,s,o,g,r,a,m){i['GoogleAnalyticsObject']=r;i[r]=i[r]||function(){
			(i[r].q=i[r].q||[]).push(arguments)},i[r].l=1*new Date();a=s.createElement(o),
			m=s.getElementsByTagName(o)[0];a.async=1;a.src=g;m.parentNode.insertBefore(a,m)
			})(window,document,'script','//www.google-analytics.com/analytics.js','ga');

			ga('create', 'UA-53004474-2', 'auto');
			ga('send', 'pageview');
		</script>
