<?php if(!isset($common_included_flag)) { require_once '/home/a6325779/public_html/old/includes/common.php'; } ?>
		<footer>
			<div class="wrapper">
			
				<ul id="footer-cols">
					
					<li class="first-col">
						
						<div class="widget-block">
							<h4>Quick Contact</h4>
							<div class="recent-post">
								<a href="mailto:<?php echo getCouncillorEmail('chair'); ?>" target="_blank" class="thumb"><img src="<?php echo getCouncillorImagename('chair'); ?>" width="50" alt="Profile" /></a>
								<div class="post-head">
									<a href="mailto:<?php echo getCouncillorEmail('chair'); ?>" target="_blank">Chair</a><span><?php echo getCouncillorName('chair'); ?></span>
								</div>
							</div>
							<div class="recent-post">
								<a href="mailto:<?php echo getCouncillorEmail('vicechair'); ?>" target="_blank" class="thumb"><img src="<?php echo getCouncillorImagename('vicechair'); ?>" width="50" alt="Profile" /></a>
								<div class="post-head">
									<a href="mailto:<?php echo getCouncillorEmail('vicechair'); ?>" target="_blank">Vice Chair</a><span><?php echo getCouncillorName('vicechair'); ?></span>
								</div>
							</div>
							<div class="recent-post">
								<a href="mailto:<?php echo getCouncillorEmail('comms'); ?>" target="_blank" class="thumb"><img src="<?php echo getCouncillorImagename('comms'); ?>" width="50" alt="Profile" /></a>
								<div class="post-head">
									<a href="mailto:<?php echo getCouncillorEmail('comms'); ?>" target="_blank">Communications</a><span><?php echo getCouncillorName('comms'); ?></span>
								</div>
							</div>
							<div class="recent-post">
								<a href="mailto:<?php echo getCouncillorEmail('commsweb'); ?>" target="_blank" class="thumb"><img src="<?php echo getCouncillorImagename('commsweb'); ?>" width="50" alt="Profile" /></a>
								<div class="post-head">
									<a href="mailto:<?php echo getCouncillorEmail('commsweb'); ?>" target="_blank">Web Design / Webmaster</a><span><?php echo getCouncillorName('commsweb'); ?></span>
								</div>
							</div>
						</div>

					</li>
					
					<li class="second-col">
						
						<div class="widget-block">
							<h4>Copyright</h4>
							<p>Copyright &#169; <?php echo Date('Y'); ?> Hills Road Sixth Form College Student Council</p>
                            This site, its content and its designs are permitted for use only by the Hills Road Sixth Form College Student Council for 2014-2015.</p><!--You should probably ask AW if you want to use them-->
							<p>Originally designed by <a href="http://www.andrew-watts.co.uk" target="_blank">Andrew Watts</a>.<br>Modified by <a href="http://www.ely-web-design.co.uk/" target="_blank">Thomas Frodsham</a>, then by <a href="//garethnunns.com">Gareth Nunns</a></p>
                            <p>Special thanks to a programming genius - Luis Zuno ("Ansimuz") - who did some of the nasty javascript coding!</p>
							
						</div>
						
					</li>
					
					<li class="third-col">
						
						<img src = "/img/studentcouncil.png" alt = "Student Council, Hills Road Sixth Form College" style = "margin: 10px 34px 0px;" /><br /><a href = "http://www.hrsfc.ac.uk/"><img src = "/img/hrsfc.png" alt = "Hills Road Sixth Form College, Cambridge" style = "margin: 40px 0px 40px 19px;" /></a><br /><p style = "text-align: center; font-size: 11px; line-height: 16px">This website is run solely by the Hills Road Sixth Form College Student Council. It does not reflect the views or opinions of Hills Road Sixth Form College.</p>

		         		
					</li>	
				</ul>				
				<div class="clearfix"></div>
				
				
			</div>
			
			<div id="to-top"></div>
		</footer>


    <!-- prettyPhoto -->
		
		<link rel="stylesheet" href="js/prettyPhoto/css/prettyPhoto.css" type="text/css" media="screen"> 
		<script src="js/prettyPhoto/js/jquery.prettyPhoto.js" type="text/javascript"></script>
        <!-- ENDS prettyPhoto -->
		
		<!-- poshytip -->
		<link rel="stylesheet" href="js/poshytip-1.1/src/tip-twitter/tip-twitter.css" type="text/css">
		<link rel="stylesheet" href="js/poshytip-1.1/src/tip-yellowsimple/tip-yellowsimple.css" type="text/css">
		<script src="js/poshytip-1.1/src/jquery.poshytip.min.js" type="text/javascript"></script>
		<!-- ENDS poshytip -->
    
    
		
		<!-- Flex Slider -->
		<link rel="stylesheet" href="css/flexslider.css" type="text/css">
		<script src="js/jquery.flexslider-min.js" type="text/javascript"></script>
		<!-- ENDS Flex Slider -->
    
    <!-- Tweet 
		<link rel="stylesheet" href="css/jquery.tweet.css" media="all" type="text/css"> 
		<script src="js/tweet/jquery.tweet.js" type="text/javascript"></script> 
		ENDS Tweet -->
		